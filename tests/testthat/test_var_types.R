...
test_that("constant S + constant estimation works", {
  f <- flash(M, S = 0.2, var.type = 0, greedy.Kmax = 1, verbose = 0)
  expect_equal(f$flash.fit$tau, f$flash.fit$given.tau)

  f <- flash(M, S = 0.05, var.type = 0, greedy.Kmax = 1, verbose = 0)
...
test_that("kronecker variance estimation works", {
  ...
  f <- flash(Y, var.type = c(1, 2), greedy.Kmax = 1, verbose = 0)
...
test_that("basic noisy variance estimation works", {
  f.const <- flash(M, var.type = 0, greedy.Kmax = 1, verbose = 0)
  f.noisy <- flash(M, S = matrix(0.01, nrow = nrow(M), ncol = ncol(M)),
                   var.type = 0, greedy.Kmax = 1, verbose = 0)
...
test_that("fixed + by_column estimation works", {
  f.bycol <- flash(M, var.type = 2, greedy.Kmax = 1, verbose = 0)
  f.noisy <- flash(M, S = (matrix(0.01, nrow = nrow(M), ncol = ncol(M))
                           + 0.001 * rnorm(length(M))),
                   var.type = 2, greedy.Kmax = 1, verbose = 0)
...
test_that("fixed + kronecker estimation works", {
  ...
  f.kron <- flash(M, var.type = c(1, 2), greedy.Kmax = 1, verbose = 0)
  f.noisy <- flash(M, S = matrix(0.01, nrow = nrow(M), ncol = ncol(M)),
                   var.type = c(1, 2), greedy.Kmax = 1, verbose = 0)
