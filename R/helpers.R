# Helpers for the flash (not flash_fit) object --------------------------------
...
# Getters for the flash_fit object (also used by the smaller factors) ---------
...
is.tau.constant <- function(f) {
  return(!is.null(get.est.tau.dim(f)) && identical(get.est.tau.dim(f), 0))
}
