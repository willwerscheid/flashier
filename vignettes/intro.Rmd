...
Finally, `flashier` provides a convenient `plot()` method. By default, `plot()` returns two figures: a "scree plot" showing the proportion of variance explained by each factor/loadings pair; and plots of posterior means of either factors or loadings (depending on the argument to `pm_which`). Here, we supply colors for the posterior means plots in order to match the figures included in Wang and Stephens (2021):
