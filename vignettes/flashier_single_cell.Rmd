---
title: "Empirical Bayes non-negative matrix factorization for scRNA-seq data"
output: 
...
+ `backfit = FALSE` skips the backfitting step. Backfitting can often
  grealy improve the fit, and is generally recommended for better
  result, but can sometimes take a long time to complete. We have
  omitted the backfitting step here to shorten the `flash()` call.
...
mu <- colMeans(shifted_log_counts)
plot_change_vs_mean <- function (fit, mu, genes, k = 1, n = 10) {
  pdat <- data.frame(mean = mu,change = ldf(fit)$F[,k],gene = genes)
  rows <- order(pdat$change,decreasing = TRUE)
  rows <- rows[1:n]
  pdat[-rows,"gene"] <- ""
  return(ggplot(pdat,aes(x = change,y = mean,label = gene)) +
...
plot_change_vs_mean(fit,mu,pbmc_facs$genes$symbol,k = 6)
```
