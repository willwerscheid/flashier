...
The default `plot` method produces a scree plot showing the proportion of
variance explained by each of the $K = 8$ fitted factors: 

```{r scree, fig.height=2, fig.width=6, results="hide", message=FALSE}
plot(fit, order_by_pve = FALSE)
```

We note that factors 7 and 8 explain a very small proportion of variation and probably
...
given by the membership levels. 
...
plot(fit, 
     ...
     pm_colors = brewer.pal(8, "Set1"),
     plot_type = "structure",
     gap = 20,
     verbose = FALSE)
```

The bottom factor k1 is present in all cell types, 
and is likely capturing a "baseline" level of expression throughout. 
...
plot(fit, 
     ...
     pm_which = "loadings", 
     pm_groups = pbmc_facs$samples$subpop,
     pm_colors = brewer.pal(8, "Set1")[2:8],
     plot_type = "structure",
     gap = 20,
     verbose = FALSE)
```
  
It is clear from this plot that factors 2 through 6
...
which suggest a subpopulation that is intermediate between
"pure" T and "pure" NK cells. It is also interesting to note the
subset of CD34+ cells that are primarily loaded on factor 3
(green); some of these cells may be mislabeled.
