...
function from [MatrixExtra][matrixextra].
...
```{r scree-plot, fig.height=2, fig.width=3}
...
In this non-negative matrix factorization produced by flashier, the
rows of the ${\bf L}$ matrix contains the estimated "membership
levels," or "memberships," for each cell. For example:
...
We can also visualize the cell matrix with a heatmap:
