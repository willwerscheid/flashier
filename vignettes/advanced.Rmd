...
gtex.nonnegative <- flashier(gtex, greedy.Kmax = 3, var.type = 1,
                             prior.class = c(prior.normal.mix(),
                                             prior.nonnegative()))
```
...
no.warmstarts.time <- system.time(
  gtex.no.warmstarts <- flashier(gtex, flash.init = gtex.nonnegative,
                                 backfit = "only", warmstart.backfits = FALSE)
)
```
...
for (k in 1:greedy.Kmax) {
  nn.fit <- flashier(gtex, flash.init = fit, greedy.Kmax = 1, var.type = 1,
                     prior.class = prior.nonnegative(),
                     output.lvl = 0, verbose.lvl = 0)
  np.fit <- flashier(gtex, flash.init = fit, greedy.Kmax = 1, var.type = 1,
                     prior.class = c(prior.nonnegative(), 
                                     prior.nonpositive()),
                     output.lvl = 0, verbose.lvl = 0)
...
gtex.same.signs <- flashier(flash.init = fit, greedy.Kmax = 0)
