...
Parameter `output.lvl` determines the form of the returned list. In most cases, the default `output.lvl = 3` will do. The other important setting is `output.lvl = 0`, which causes `flashier` to return a flash fit that is entirely free from post-processing and can be fed directly into subsequent `flashier` calls. This can save time in complex flash fits that involve repeated calls to `flashier`.
...
for (k in 1:greedy.Kmax) {
  ...
  if (nn.fit$obj > np.fit$obj) {
    message("Nonnegative factor added. Objective: ",
            formatC(nn.fit$obj, format = "f", digits = 2))
    fit <- nn.fit
  } else {
    message("Nonpositive factor added. Objective: ",
